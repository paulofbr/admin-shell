...
export async function getRoles(): Promise<Role[]> {
  const response = await api.getApiV1Roles()
  return response.data.map(normalizeRole)
}

export async function getRoleById(id: string): Promise<Role> {
  const response = await api.getApiV1RolesId(id)
...
export async function createRole(
  ...
  const response = await api.postApiV1Roles({
    name: data.name,
...
export async function updateRole(
  ...
  const response = await api.putApiV1RolesId(id, {
    name: data.name ?? '',
    description: data.description ?? null,
    extensionFields: data.extensionFields,
...
export async function deleteRole(id: string): Promise<void> {
  await api.deleteApiV1RolesId(id)
}

export async function getAllPermissions(): Promise<Permission[]> {
  const response = await api.getApiV1RolesPermissions()
  return response.data as Permission[]
}

export async function getRolePermissions(id: string): Promise<RolePermissionsResponse> {
  const response = await api.getApiV1RolesIdPermissions(id)
...
export async function assignPermission(roleId: string, permissionId: string): Promise<void> {
  await api.postApiV1RolesIdPermissions(roleId, { permissionId })
}

export async function removePermission(roleId: string, permissionId: string): Promise<void> {
  await api.deleteApiV1RolesIdPermissionsPermissionId(roleId, permissionId)
}
