...
public class DatabaseInitializer
{
    ...
    private async Task SeedDataAsync(IDbConnection db)
    {
        ...
        {
            _logger.LogInformation("Database already seeded, skipping role/user seed");
        }
        else
        {
            // Seed roles
            ...
            await db.ExecuteAsync(
                ...
                new { Id = userId, Email = "admin@admin.com", Username = "admin", DisplayName = (string?)"Administrator", Pwd = passwordHash, Now = now });

            // Assign admin role to admin user
            await db.ExecuteAsync(
                "INSERT INTO UserRoles (UserId, RoleId) VALUES (@UserId, @RoleId)",
                new { UserId = userId, RoleId = roleAdminId });

            // Seed some permissions
            var perms = new[]
            {
                new { Id = Guid.NewGuid(), Code = "users:read", Resource = "users", Action = "read" },
                new { Id = Guid.NewGuid(), Code = "users:write", Resource = "users", Action = "write" },
                new { Id = Guid.NewGuid(), Code = "users:delete", Resource = "users", Action = "delete" },
                new { Id = Guid.NewGuid(), Code = "plugins:manage", Resource = "plugins", Action = "manage" },
                new { Id = Guid.NewGuid(), Code = "settings:read", Resource = "settings", Action = "read" },
                new { Id = Guid.NewGuid(), Code = "settings:write", Resource = "settings", Action = "write" },
            };

            await db.ExecuteAsync(
                @"INSERT INTO Permissions (Id, Code, Resource, Action, IsDeleted, CreatedAt, CreatedBy)
                  VALUES (@Id, @Code, @Resource, @Action, 0, @Now, 'system')",
                perms.Select(p => new { p.Id, p.Code, p.Resource, p.Action, Now = now }).ToList());

            // Assign all permissions to admin role
            await db.ExecuteAsync(
                "INSERT INTO RolePermissions (RoleId, PermissionId) VALUES (@RoleId, @PermissionId)",
                perms.Select(p => new { RoleId = roleAdminId, PermissionId = p.Id }).ToList());

            _logger.LogInformation("Database seeded: admin@admin.com / admin123");
        }
